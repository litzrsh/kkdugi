...
class PragmaControllerTest {
    ...
    void cleanUp() {
        ...
        jdbcTemplate.update("DELETE FROM kkdugi_menu_lang WHERE menu_id IN (?, ?)", MENU_ID, NO_PROGRAM_MENU_ID);
        jdbcTemplate.update("DELETE FROM kkdugi_menu_base WHERE menu_id IN (?, ?)", MENU_ID, NO_PROGRAM_MENU_ID);
