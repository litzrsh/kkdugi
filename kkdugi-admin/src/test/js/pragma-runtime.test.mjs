...
test('Pragma uses authenticated text request with context path and no redirect/cache',async()=>{
 ...
 const api=pageApi({basePath:'/kk/'},async(url,options)=>{captured={url,options};return new Response('<template><p>Screen</p></template>',{headers:{'Content-Type':'text/html;charset=UTF-8'}});});
...
test('Pragma refuses login HTML and reports 404 without pretending it is a session failure',async()=>{
 const api=pageApi({},async()=>new Response('<!doctype html><html>Login</html>',{headers:{'content-type':'text/html'}}));
 await assert.rejects(api.pragma('M'),e=>e.status===502);
 setTokenCookie('still-present');const missing=pageApi({},async()=>new Response(null,{status:404}));
...
test('401 with non-JSON body still clears token and calls disposal',async()=>{
 ...
 const api=pageApi({onUnauthorized:()=>disposed=true},async()=>new Response('<html>unauthorized</html>',{status:401}));
...
test('batch empty success and both error formats are preserved',async()=>{
 const empty=pageApi({},async()=>new Response(''));assert.equal(await empty.persist('code',{}),null);
 for(const body of [{errors:[{code:'x',reason:'code.err.duplicate'}]},[{code:'err.default',message:'fail'}]]){
 const api=pageApi({},async()=>new Response(JSON.stringify(body),{status:400}));await assert.rejects(api.persist('i18n',{}),e=>e.errors.length===1);
 }
 assert.throws(()=>empty.list('user',{}),/Unsupported/);
});
test('single backend error retains its localized message',async()=>{
 const api=pageApi({},async()=>new Response(JSON.stringify({code:'code.err.duplicate',message:'Duplicate code'}),{status:409}));
