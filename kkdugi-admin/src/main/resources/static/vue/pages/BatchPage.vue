...
const s=inject('kkdugi');const {t,state,api,clone,payload,rowState,localeValue,validate,flatten,isProtected,prepareLocalizedRow,prepareBatch,menuDescendants,filterMenuRows}=s;
...
const columns=computed(()=>{
 ...
 list.push({headerName:t('label'),width:270,valueGetter:p=>localeValue(p.data,state.locale,'label'),cellRenderer:p=>{const el=document.createElement('span');el.className='name-cell';el.style.paddingLeft=(p.data._depth||0)*18+'px';el.textContent=p.value||'—';return el;}},simple('program','program',160),simple('path','path',230),simple('use','use',100),simple('close','closable',120),simple('sort','sort',95));
 }else{
...
async function editRow(row=null,parent=null){
 ...
 if(menu){
  initial._parentLabel=localeValue(rows.value.find(r=>r.id&&r.id===initial.parentId),state.locale,'label')||t('root');
  add('_parentLabel','parent','text',{readonly:true});add('program','program','text',{readonly:fixed});add('icon','icon','text',{readonly:fixed});
...
async function removeRows(targets){
 if(!canDelete.value||!targets.length)return;const menu=props.screen==='admmenu';
 ...
 const group=crypto.randomUUID();for(const row of menu?menuDescendants(rows.value,targets):targets)if(!row._deleted){row._deleted=true;row._deleteGroup=group;}
 rows.value=rows.value.filter(r=>!(r._deleted&&!baseline.value.some(b=>b._key===r._key)));selected.value=[];grid.value?.clear();tick.value++;
}
function restore(row){if(!canDelete.value)return;const group=row._deleteGroup;for(const item of rows.value)if(item._key===row._key||(group&&item._deleteGroup===group)){delete item._deleted;delete item._deleteGroup;}rows.value=[...rows.value];tick.value++;}
...
async function save(throwError=false){
 grid.value?.stop();tick.value++;let batch=changes.value;if(!changeCount.value)return;saving.value=true;error.value='';
 try{
