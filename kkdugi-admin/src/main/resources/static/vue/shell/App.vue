...
const requests=createPageRequests(s.runtime.page);let disposed=false,menuRequest,committedHash='',pendingMenu=null,transition=0,guardPromise,localeChanging=false,missingScreenAlert;
...
function showMissingScreen(){
 transition++;requests.cancel();loading.value=false;mobileNav.value=false;pendingMenu=null;screenError.value='';restoreHash();
 if(!missingScreenAlert)missingScreenAlert=s.dialog.alert({title:t('notice'),message:t('screen_not_found')}).finally(()=>{missingScreenAlert=null;});
 return missingScreenAlert;
}
async function navigate(menu,fromHistory=false){
 ...
 catch(e){if(ticket===transition&&!disposed){if(e.status===404)await showMissingScreen();else{screenError.value=s.errorText(e);restoreHash();}}}
 finally{if(ticket===transition)loading.value=false;}
}
async function readHash(){const id=menuIdFromHash(location.hash);if(id===null)return showMissingScreen();if(!id){await navigate(null,true);return;}const menu=findMenu(menus.value,id);if(!menu||menu.children.length)return showMissingScreen();await navigate(menu,true);}
async function loadMenus(){
 menuRequest?.abort();const controller=new AbortController();menuRequest=controller;menuLoading.value=true;menuError.value='';
 try{const result=normalizeMenus(await s.api.menus({locale:state.locale,signal:controller.signal}));if(disposed||controller!==menuRequest)return;menus.value=result;await readHash();}
...
async function changeLocale(event){
 const locale=event.target.value;event.target.value=state.locale;
 if(localeChanging||locale===state.locale)return;
 localeChanging=true;
 try{if(!await guard('locale')||disposed)return;const url=new URL(location.href);url.searchParams.set('lang',locale);location.assign(url.href);}
 finally{localeChanging=false;}
}
