...
- **마지막 `SYS_ADMIN` 사용자 보호는 하지 않는다 (오너 결정, 2026-09-19).** 사용자 삭제(5.8),
  ...
  "경고" 문단과 같은 위험이다.
- **사용자 상태는 아직 로그인에 반영되지 않는다.** `SessionUser`의 `isSuspended()` 등은 사용처가
  없고 `isEnabled()`도 재정의하지 않아, 정지(50)·탈퇴(40)·휴면(30) 사용자도 지금은 로그인된다.
  5.9의 상태 변경은 값을 저장할 뿐 강제하지 않는다. 별도 후속 작업이다.
...
- 사용자 본인의 비밀번호 변경 API, 로그인 실패 잠금, 비밀번호 정책(복잡도·만료 갱신)은 범위 밖이다.
