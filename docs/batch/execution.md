...
- 상태: 2026-09-20 설계. URL·요청·응답은 [Runner API](runner-api.md)와 [관리자 API](admin-api.md)에서 정의하며 DB enum 코드는 DDL 단계에서 확정한다. API는 아직 미구현이다.
...
입력은 크기가 제한된 JSON 객체다. v1은 JSON_FILE 모드로 입력 파일을 만들고 `KKDUGI_INPUT_FILE`/`KKDUGI_RESULT_FILE` 환경 변수로 위치를 전달한다. 자유로운 argv 매핑은 후속 버전이며 고정 wrapper로 기존 프로그램과 연결할 수 있다. 비밀 값은 runner가 현지에서 해석하며 admin의 Run·로그·Event에 평문 저장하지 않는다. 로그 마스킹만으로 유출을 완전히 방지할 수 없으므로 프로그램도 비밀 값을 출력하지 않아야 한다.
