...
- **마지막 `SYS_ADMIN` 사용자 보호가 없다(오너 결정).** 삭제(8), 권한 저장(6), 그리고 [권한 API](authority.md)의 `users`로
  마지막 시스템 관리자를 없앨 수 있고, 그러면 DB를 직접 고치는 것 외에 복구 방법이 없다. 상태 변경(9)으로 마지막 시스템 관리자를 비정상 상태로 바꾸어도 이후 로그인할 수 없다.
- 사용자 상태는 이후 로그인부터 적용된다. 정상(20)만 로그인 가능하며, 이미 접속 중인 세션은 상태 변경만으로 종료되지 않는다. [로그인 상태 정책](auth.md)을 참조한다.
- 임시 비밀번호 평문 로그는 메일 발송이 구현될 때까지의 임시 조치다.
- 초기/만료 비밀번호 변경과 30일 연장은 [로그인 API](auth.md)에서 처리한다. 로그인 후 별도의 비밀번호 변경 API와 실패 횟수 잠금은 미구현이다.
