...
기능은 (당시 ERD에는 존재했지만) 이번 구현 범위에서 명시적으로 제외한다.
...
`KKDUGI_I18N_MSG`는 Flyway `V1__create_i18n_msg.sql`이 정의한다(원래 기준이던
`erd/`는 2026-09-20에 삭제되어 마이그레이션이 스키마의 기준이다). DB 컬럼명은
ERD 시절부터 변경되지 않았다.
